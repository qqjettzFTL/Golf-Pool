...
const JoinPoolPage = () => {
  ...
  setError('');
  try {
      const response = await axios.get(`http://localhost:3001/api/pools/${poolName}`);
      const pool = response.data;
  
      if (pool.pool_password === poolPassword) {
      setSuccess(true);
      } else {
      ...
      setError('Pool not found. Please check the name and try again.');
  }
  };
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="flex-grow flex flex-col items-center justify-center space-y-6 p-4 text-center">
        <h1 className="text-2xl font-semibold text-gray-800 leading-relaxed">
          Join an existing<br />PGA Draft Pool
        </h1>

        <form onSubmit={handleJoinPool} className="space-y-6 w-full max-w-sm">
        ...
        {success && (
        <div className="mt-4">
            <p className="text-green-600 font-semibold mb-2">You have successfully joined the pool!</p>
            <button
